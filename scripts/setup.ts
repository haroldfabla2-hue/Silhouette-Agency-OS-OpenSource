...
async function findAvailablePort(preferred: number, name: string): Promise<number> {
    ...
    if (available) {
        print(`  ✅ Port ${preferred} (${name}): Available`);
        return preferred;
    }
    ...
    for (let offset = 1; offset <= 20; offset++) {
        ...
        if (await checkPort(alt)) {
            print(`  ⚠️  Port ${preferred} (${name}): In use → Using ${alt}`);
            return alt;
        }
    }

    print(`  ❌ No available port found for ${name} near ${preferred}`);
...
async function main() {
    printHeader('🌙 SILHOUETTE AGENCY OS — INTELLIGENT SETUP');
    ...
    print(`  Node.js: ${hasNode ? '✅ Found' : '❌ Not found'}`);
    print(`  npm:     ${hasNpm ? '✅ Found' : '❌ Not found'}`);
    print(`  Docker:  ${hasDocker ? '✅ Found' : '⚠️  Not found (optional, for Neo4j/Redis/Qdrant)'}`);

    if (!hasNode || !hasNpm) {
        print('\n❌ Node.js and npm are required. Please install Node.js 18+ and try again.');
        process.exit(1);
    }
        ...
        if (response.toLowerCase() === 'y') {
            if (provider.requiresKey) {
                ...
                if (key) {
                    ...
                    print(`  ✅ ${provider.name} configured`);
                } else {
                    print(`  ⚠️  Skipped ${provider.name} (no API key)`);
                }
            } else {
                ...
                print(`  ✅ ${provider.name} configured (local)`);
            }
        }
    }

    if (selectedProviders.length === 0) {
        print('\n⚠️  No providers selected. You can configure them later in silhouette.config.json');
    }
    ...
    print(`  ✅ Created silhouette.config.json`);

    // Write .env.local
    const envLines: string[] = [
        `# Generated by Silhouette Setup — ${new Date().toISOString()}`,
        `PORT=${ports.api}`,
    ...
    if (fs.existsSync(ENV_PATH)) {
        print(`  ⚠️  .env.local already exists — skipping (won't overwrite your keys)`);
    } else {
    ...
    try {
        ...
        print('  ⚠️  npm install failed. Attempting npm audit fix...');
        try {
            execSync('npm audit fix', { cwd: PROJECT_ROOT, stdio: 'inherit' });
            execSync('npm install', { cwd: PROJECT_ROOT, stdio: 'inherit' });
            print('  ✅ Dependencies fixed and installed');
        } catch {
            print('  ❌ Dependency installation failed. Run "npm install" manually.');
        }
    }
    ...
    if (hasDocker) {
        ...
        if (startDocker.toLowerCase() === 'y') {
            try {
                ...
                print('  ⚠️  Docker compose failed. Start containers manually.');
            }
        }
    }
